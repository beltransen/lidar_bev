...
def get_max_points_map(bv_size, cell_size, z_min, z_max, nplanes, low_angle, h_res, v_res, lidar_height):
    grid_size = int(bv_size / cell_size)
    print 'Generating map with size: {} x {} '.format(grid_size, grid_size)
    max_map = np.zeros((grid_size, grid_size), dtype=np.float)
    ...
    for i in xrange(0, int(grid_size/2)):  # i -> row (y-axis velo plane / x-axis image plane)
        for j in xrange(0, i+1): # j -> col (x-axis velo plane / y-axis image plane)
            ...
            if x_min == 0 and y_min == 0: # Closest cells takes all points
                max_map[int(grid_size/2)-1][int(grid_size/2)-1]=nplanes*num_hor_rays
                max_map[int(grid_size/2)-1][int(grid_size/2)]=nplanes*num_hor_rays
                max_map[int(grid_size/2)][int(grid_size/2)]=nplanes*num_hor_rays
                max_map[int(grid_size/2)][int(grid_size/2)-1]=nplanes*num_hor_rays
                ...
                for p in xrange(0, nplanes):
                    ...
                    while current_hangle <change_angle:
                        ...
                        if z_min <= z <= z_max:
                            ...
                            max_map[int(grid_size/2)-1-j][int(grid_size/2)-1-i]+=1
                            max_map[int(grid_size/2)-1-j][int(grid_size/2)+i]+=1
                            max_map[int(grid_size/2)+j][int(grid_size/2)+i]+=1
                            max_map[int(grid_size/2)+j][int(grid_size/2)-1-i]+=1

                            if i != j:
                                max_map[int(grid_size/2)-1-i][int(grid_size/2)-1-j]+=1
                                max_map[int(grid_size/2)-1-i][int(grid_size/2)+j]+=1
                                max_map[int(grid_size/2)+i][int(grid_size/2)+j]+=1
                                max_map[int(grid_size/2)+i][int(grid_size/2)-1-j]+=1
...
if __name__ == '__main__':
    ...
    max_map = get_max_points_map(map_size, cell_size, min_height, max_height, num_planes, velo_minangle, velo_hres, velo_vres, velo_height)
    print 'max {} min {}'.format(np.max(max_map), np.min(max_map))

    cell_str = '{0:.2f}'.format(cell_size)
    velo_height_str = '{0:.2f}'.format(velo_height)
    print 'Map created at {}/{}_{}_{}_{}_map.txt'.format(path, map_size, cell_str, num_planes, velo_height_str)
    np.savetxt('{}/{}_{}_{}_{}_map.txt'.format(path, map_size, cell_str, num_planes, velo_height_str), max_map) # fmt='%.2f'
